...
if ($method === 'POST' && isset($_GET['delete_message'])) {
    try {
        ...
        // Handle empty input
        if (empty($rawInput)) {
            respond(['error' => 'Request body is required'], 400);
        }
        ...
        if (json_last_error() !== JSON_ERROR_NONE) {
            respond(['error' => 'Invalid JSON: ' . json_last_error_msg()], 400);
        }
        
        // Handle case where input might be null or empty
        if (!$input || !is_array($input)) {
            respond(['error' => 'Invalid request body format'], 400);
        }
        
        // Extract message_id - try multiple possible field names
        $message_id = 0;
        if (isset($input['message_id'])) {
            $message_id = is_numeric($input['message_id']) ? intval($input['message_id']) : 0;
        } else if (isset($input['id'])) {
            // Some frontends might send 'id' instead of 'message_id'
            $message_id = is_numeric($input['id']) ? intval($input['id']) : 0;
        }
        
        // Validate message_id
        if ($message_id <= 0) {
            logMessage("Delete message failed - Invalid message_id: " . ($input['message_id'] ?? $input['id'] ?? 'missing'));
            respond(['error' => 'Invalid message_id: must be a positive integer', 'received' => $input], 400);
        }
        
        // Verify message exists before deleting
        $checkStmt = $pdo->prepare('SELECT id FROM chat_messages WHERE id = ?');
        ...
        if ($stmt->rowCount() > 0) {
            ...
            respond(['error' => 'Failed to delete message: No rows affected'], 500);
        }
        
    } catch (Exception $e) {
        logMessage("Delete message exception: " . $e->getMessage());
