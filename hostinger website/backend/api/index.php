...
function handleDeleteComponent($pdo) {
    ...
    // Accept id from JSON body, form body, or query string, supporting common aliases
    $candidates = [
        $input['id']            ?? null,
    ...
    // Fallback: sometimes clients send a plain text body like "1168"
...
function handleRestoreComponent($pdo) {
	...
	$id = is_numeric($idParam) ? (int)$idParam : null; // allow 0 as valid archive id
    ...
    try {
        ...
        // Get the archived component by archive id or original_id
        if ($id !== null) {
        ...
        if (!$archived) {
            // Graceful idempotency: if the archived row is not found, consider that it may already be restored.
            // Try to detect using the supplied original_id (preferred) or fall back to the numeric id.
            $maybeOriginalId = $originalId > 0 ? $originalId : (is_numeric($id) ? (int)$id : 0);
            if ($maybeOriginalId > 0) {
                $existsLive = $pdo->prepare("SELECT 1 FROM components WHERE id = ?");
                $existsLive->execute([$maybeOriginalId]);
                if ($existsLive->fetchColumn()) {
                    if ($pdo->inTransaction()) { $pdo->rollBack(); }
                    echo json_encode(['success' => true, 'message' => 'Component already restored']);
                    return;
                }
            }
            if ($pdo->inTransaction()) { $pdo->rollBack(); }
		...
		// Determine the target id to restore as (preserve original id)
        ...
        // Instead of failing, auto-rename the restored component to avoid conflicts.
        $nameToInsert = $archived['name'];
        try {
            $checkStmt = $pdo->prepare("SELECT id FROM components WHERE name = ? AND id != ?");
            $checkStmt->execute([$archived['name'], $archived['original_id'] ?? 0]);
            if ($checkStmt->fetch()) {
                // Auto-suffix with a safe, short marker
                $suffix = ' (restored ' . date('Ymd-His') . ')';
                // Ensure we don't exceed typical VARCHAR(255) limits
                if (mb_strlen($nameToInsert . $suffix) > 255) {
                    $nameToInsert = mb_substr($nameToInsert, 0, 255 - mb_strlen($suffix));
                }
                $nameToInsert .= $suffix;
            }
        } catch (Throwable $t) {
            // Non-fatal: fallback to original name if check fails
            $nameToInsert = $archived['name'];
        }
		...
		// Build INSERT statement dynamically based on available columns, preserving id
        if ($hasWarranty) {
        ...
        // Delete from archive
        // If we don't have the archive row id, delete by original_id as a fallback.
        if ($id !== null) {
            $deleteStmt = $pdo->prepare("DELETE FROM archive_components WHERE id = ?");
            $deleteStmt->execute([$id]);
        } else {
            $deleteStmt = $pdo->prepare("DELETE FROM archive_components WHERE original_id = ? ORDER BY deleted_at DESC LIMIT 1");
...
function handleGetComponents($pdo) {
    ...
    $onlyInStock = isset($_GET['only_in_stock']) && ($_GET['only_in_stock'] === '1' || strtolower($_GET['only_in_stock']) === 'true');
    ...
    // Optional post-filter: only return in-stock components when requested
    if ($onlyInStock) {
        $components = array_values(array_filter($components, function($c) {
            // Treat missing stock as 0 for safety
            $qty = isset($c['stock_quantity']) ? (int)$c['stock_quantity'] : 0;
            return $qty > 0;
        }));
    }
...
switch ($endpoint) {
    ...
    case 'admin_change_password':
        if ($method === 'PUT') {
            ...
            logSecurityEvent($pdo, 'admin_password_change', "Super Admin changed password for user ID: $userId", $decoded['user_id'], $_SERVER['REMOTE_ADDR'] ?? 'unknown');
