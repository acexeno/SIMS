...
if ($method === 'GET' && isset($_GET['stats'])) {
    try {
        ...
        $stats['total_sessions'] = intval($stmt->fetchColumn());
        
        // Open sessions
        $stmt = $pdo->query('SELECT COUNT(*) FROM chat_sessions WHERE status = "open"');
        $stats['open_sessions'] = intval($stmt->fetchColumn());
        
        // Today's sessions
        $stmt = $pdo->query('SELECT COUNT(*) FROM chat_sessions WHERE DATE(created_at) = CURDATE()');
        $stats['today_sessions'] = intval($stmt->fetchColumn());
        
        // Total unread messages (from user to admin/employee)
        $stmt = $pdo->query('SELECT COUNT(*) FROM chat_messages WHERE sender = "user" AND read_status = "unread"');
        $stats['total_unread_messages'] = intval($stmt->fetchColumn());
...
if ($method === 'GET' && isset($_GET['messages']) && isset($_GET['session_id'])) {
    try {
        ...
        // Ensure message IDs are integers
        foreach ($messages as &$message) {
            $message['id'] = intval($message['id'] ?? 0);
        }
        unset($message);
        ...
        if ($user_id) {
            ...
            // For staff viewers with chat access, mark user messages as read
            $viewer = getUserFromToken();
            if ($viewer && hasChatPermission($viewer, 'read')) {
                $stmt = $pdo->prepare('
                    UPDATE chat_messages 
                    SET read_status = "read" 
                    WHERE session_id = ? AND sender = "user" AND read_status = "unread"
                ');
                $stmt->execute([$session_id]);

                // Record last seen timestamp for the staff member (if we can resolve their ID)
                $viewerId = $viewer['user_id'] ?? $viewer['id'] ?? null;
                if ($viewerId) {
                    $stmt = $pdo->prepare('
                        INSERT INTO last_seen_chat (user_id, session_id, last_seen_at) 
                        VALUES (?, ?, NOW()) 
                        ON DUPLICATE KEY UPDATE last_seen_at = NOW()
                    ');
                    $stmt->execute([$viewerId, $session_id]);
                }
            }
        }
...
if ($method === 'POST' && isset($_GET['send'])) {
    try {
        ...
        // Validate session ownership similar to messages endpoint
        $query = 'SELECT id FROM chat_sessions WHERE id = ?';
        $params = [$session_id];
        ...
        // For guests, verify the session belongs to them
            ...
            if ($guest_name && !empty($guest_email)) {
                ...
                $query .= ' AND guest_name = ? AND guest_email = ?';
                $params[] = $guest_name;
                $params[] = $guest_email;
            } else if (!empty($guest_email)) {
                // Only email provided: match on email
                ...
                // Only name provided: match on name (and ensure email is also NULL for this session)
                $query .= ' AND guest_name = ? AND guest_email IS NULL';
                $params[] = $guest_name;
            }
        }
        
        $checkSession = $pdo->prepare($query);
        $checkSession->execute($params);
...
if ($method === 'GET' && isset($_GET['unread_count'])) {
    try {
        ...
        respond(['success' => true, 'unread_count' => intval($count)]);
