...
function handleRegister($pdo) {
    ...
    try {
        ...
        if (!$executeResult) {
            ...
            throw new Exception('INSERT execution failed: ' . ($errorInfo[2] ?? 'Unknown error'));
        }
        
        $userId = $pdo->lastInsertId();
        
        if (!$userId || $userId === '0') {
            // Check if the insert actually succeeded by querying the user
            ...
            if ($existing) {
                $userId = $existing['id'];
                error_log("Warning: lastInsertId() returned 0 but user exists with ID: " . $userId);
            } else {
                $errorInfo = $pdo->errorInfo();
                throw new Exception('Failed to create user record - lastInsertId returned 0. SQL Error: ' . ($errorInfo[2] ?? 'Unknown error') . ' | Error Code: ' . ($errorInfo[0] ?? 'N/A'));
            }
        }
        ...
        if (!$role) {
            ...
            if (!$roleId || $roleId === '0') {
                throw new Exception('Failed to create role - lastInsertId returned 0');
            }
        } else {
            $roleId = $role['id'];
        }
        ...
        $pdo->rollBack();
        error_log("User registration PDO error: " . $e->getMessage() . " | SQL State: " . $e->getCode() . " | Error Info: " . json_encode($e->errorInfo ?? []));
        http_response_code(500);
        $errorMessage = $e->getMessage();
        // Include SQL error code for debugging
        if ($e->getCode()) {
            $errorMessage .= " (SQL Error Code: " . $e->getCode() . ")";
        }
        echo json_encode([
            'error' => 'Failed to create user account', 
            'details' => $errorMessage,
            'debug_info' => env('APP_DEBUG') === '1' ? [
                'sql_state' => $e->getCode(),
        ...
        $pdo->rollBack();
        error_log("User registration error: " . $e->getMessage() . " | Trace: " . $e->getTraceAsString());
        http_response_code(500);
        echo json_encode([
            'error' => 'Failed to create user account', 
