...
// Check APP_ENV - but prioritize production if we're on a real domain
$appEnv = env('APP_ENV', '');
if ($appEnv && strtolower($appEnv) === 'local' && $isLocal) {
    $isLocal = true;
} elseif ($appEnv && strtolower($appEnv) === 'production') {
    $isLocal = false;
} elseif ($host && !$isLocal && (strpos($host, '.') !== false)) {
    // If we're on a real domain (contains dots), treat as production
    $isLocal = false;
}

// Debug logging
error_log('[MAILER ENV DEBUG] Host: "' . $host . '"');
error_log('[MAILER ENV DEBUG] APP_ENV: "' . $appEnv . '"');
error_log('[MAILER ENV DEBUG] isLocal: ' . ($isLocal ? 'true' : 'false'));
...
function buildGmailMailer(): array {
    ...
    if ($authMode === '') {
        // Check for OAuth2 credentials - all must be present AND non-empty
        $hasOAuth = env('GMAIL_USER', '') && 
                   env('GMAIL_CLIENT_ID', '') && 
                   env('GMAIL_CLIENT_SECRET', '') && 
                   env('GMAIL_REFRESH_TOKEN', '');
        
        // Check for password credentials - user and password must be present AND non-empty
        $hasPass = (env('GMAIL_USER', '') && env('GMAIL_APP_PASSWORD', '')) || 
