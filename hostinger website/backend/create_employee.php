...
try {
    ...
    // 4. Insert the employee user with permissions enabled
    // Employee users should have all access permissions enabled by default
    $stmt = $pdo->prepare('INSERT INTO users (username, email, password_hash, first_name, last_name, is_active, can_access_inventory, can_access_orders, can_access_chat_support, created_at) VALUES (:username, :email, :password_hash, :first_name, :last_name, 1, 1, 1, 1, NOW())');
    $stmt->execute([
        'username' => $username,
        'email' => $email,
