...
function handleCreateOrder($pdo) {
    ...
    // Optional customer contact info passed from checkout/employee/admin forms
    ...
    try {
        ...
        // Insert financial details - set purchase_date and optional customer fields for new orders
        if ($purchaseDate) {
            // Try to include customer_name/customer_phone if columns exist
            if ($customerName !== null || $customerPhone !== null) {
                $stmt = $pdo->prepare("INSERT INTO order_financial_details (order_id, purchase_date, customer_name, customer_phone) VALUES (?, ?, ?, ?)");
                $stmt->execute([$orderId, $purchaseDate, $customerName, $customerPhone]);
            } else {
                $stmt = $pdo->prepare("INSERT INTO order_financial_details (order_id, purchase_date) VALUES (?, ?)");
                $stmt->execute([$orderId, $purchaseDate]);
            }
        } elseif ($customerName !== null || $customerPhone !== null) {
            // If purchaseDate missing but contact is provided, still create a row to store contact
            $stmt = $pdo->prepare("INSERT INTO order_financial_details (order_id, customer_name, customer_phone) VALUES (?, ?, ?)");
            $stmt->execute([$orderId, $customerName, $customerPhone]);
        }
...
function handleGetOrder($pdo) {
    ...
    try {
        ...
        // Fetch financial details including purchase_date and contact info
        ...
        if ($financialDetails) {
            ...
            if (isset($financialDetails['customer_name'])) {
                $order['customer_name'] = $financialDetails['customer_name'];
            }
            if (isset($financialDetails['customer_phone'])) {
                $order['customer_phone'] = $financialDetails['customer_phone'];
            }
        }
...
function handleUpdateOrderStatus($pdo) {
    ...
    try {
        ...
        // Update or insert financial details if purchase_date or customer fields are provided
