...
function get_db_connection() {
    ...
    // All credentials must come from environment variables (.env file)
    // If credentials are missing, throw an error instead of using defaults
    if (empty($host) || empty($db) || empty($user)) {
        error_log('[DB] Missing required database credentials in environment');
        http_response_code(500);
        $message = ($debug === '1' || strtolower($debug) === 'true')
            ? 'Database credentials not configured. Please set DB_HOST, DB_NAME, and DB_USER in .env file.'
